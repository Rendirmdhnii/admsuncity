...
export default function Editor() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-100">
      ...
      {/* ── Sub-header (Smart Document Bar & Tab Switcher) ────────────────────── */}
      <div className="bg-white border-b border-slate-200 px-4 py-3 z-20 shadow-sm">
        <div className="max-w-lg mx-auto">
          {/* Smart Document Bar */}
          <div className="bg-white/80 backdrop-blur-xl border border-slate-200/80 rounded-2xl p-3.5 mb-3 shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-3">
              {/* Icon Dokumen Aktif */}
              <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center font-bold shadow-inner shrink-0">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
              </div>
              <div className="min-w-0">
                <p className="text-[10px] uppercase tracking-wider font-bold text-slate-400">Dokumen Sedang Disunting</p>
                <h2 className="text-sm font-extrabold text-slate-800 truncate">{jenisOpt?.title || 'Perjanjian Sewa'}</h2>
              </div>
            </div>

            {/* Tombol Ganti Dokumen yang Sangat Jelas & Mudah Ditekan */}
            <button 
              onClick={gantiJenis}
              className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-3.5 py-2 rounded-xl font-bold text-xs flex items-center gap-1.5 transition-all active:scale-95 border border-slate-200 shadow-sm shrink-0 cursor-pointer"
            >
